    ...
    with UtilityFunctions.get_connection() as cursor:
        ...
        meal_stats = {
            row[0]: {
                ...
                "non-veg": res[2] if (res and res[2] is not None) else 0,
            } 
            for row in res
        }
...
def generate_new_token(total_tokens: int) -> Dict[str, Union[str, Any]]:
    with UtilityFunctions.get_connection() as cursor:
...
def verify_scanned_token(token_id: str) -> Dict[str, Union[str, int]]:
    ...
    with UtilityFunctions.get_connection() as cursor:
        ...
        return UtilityFunctions.verify_token_and_supply_data(cursor, token_id=token_id, token_number=token_number)
